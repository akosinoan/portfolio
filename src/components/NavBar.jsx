        ...
        <nav className="flex justify-between items-center h-20 bg-red-950 text-white">
            <div className="px-5">Logo </div>    
                ...
                <section className="MOBILE-MENU  lg:hidden">
                    ...
                    <div className={ "flex flex-col w-1/2 py-4 ease-in transition-all duration-300 justify-evenly items-center absolute  border-white  z-50  bg-red-950 rounded-b-3xl border-2 shadow-xl" +
                        (isNavOpen ? " top-0 left-[55%]" : "overflow-hidden -top-10 left-[100%]") }>
                        <div
...
const NavLink = (props) =>{
    ...
    return (
            ...
            <li className={` ${activeNav} group pr-5`}>
                <span className={`${activeNav===props.id?"text-yellow-200":""} group-hover:text-yellow-200 active:text-yellow-200`}> 
                    <a href={props.href} onClick={()=>handleNavClick(props.id)}>{props.name}</a>
                </span>
                <span className={`${activeNav===props.id?"bg-yellow-200":""} block h-0.5 w-full bg-transparent group-hover:bg-yellow-200 active:bg-yellow-200`}></span>
            </li>
